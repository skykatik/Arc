...
public class AntScriptGenerator{
    ...
    public void generate(BuildConfig config, BuildTarget... targets){
        ...
        if(!config.jniDir.child("memcpy_wrap.c").exists()){
            new FileHandle("scripts/memcpy_wrap.c", FileType.Classpath).copyTo(config.jniDir
            .child("memcpy_wrap.c"));
        }
        ...
        for(BuildTarget target : targets){
            ...
            FileHandle libsDir = new FileHandle(getLibsDirectory(config, target));
        ...
        String template = new FileHandle("scripts/build.xml.template", FileType.Classpath)
        .readString();
        ...
        for(String buildFile : buildFiles){
            clean.append("\t\t<ant antfile=\"" + buildFile + "\" target=\"clean\"/>\n");
            compile.append("\t\t<ant antfile=\"" + buildFile + "\"/>\n");
        }
    ...
    private void copyJniHeaders(String jniDir){
        ...
        for(String file : files){
            new FileHandle(pack, FileType.Classpath).child(file).copyTo(new FileHandle(jniDir).child("jni-headers").child(file));
        }
    }
    ...
    private String generateBuildTargetTemplate(BuildConfig config, BuildTarget target){
        ...
        if(target.os == TargetOs.Android){
            ...
            String template = new FileHandle("scripts/build-android.xml.template",
        ...
        if(target.os == TargetOs.IOS){
            template = new FileHandle("scripts/build-ios.xml.template", FileType.Classpath)
            .readString();
        }else{
            template = new FileHandle("scripts/build-target.xml.template", FileType.Classpath)
            .readString();
        }
